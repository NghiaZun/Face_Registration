...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.mean[0] = 104;
    input_meta_tab[0].image.mean[1] = 117;
    input_meta_tab[0].image.mean[2] = 123;
...
static void _load_input_meta_resnet34()
{
    uint32_t i;
    for (i = 0; i < INPUT_META_NUM; i++)
    {
...
static uint8_t *_get_jpeg_data
    ...
    vnn_input_meta_t *meta,
    const char *filename,
    uint8_t* rgbData //Add by yingwei
    )
{
    uint32_t i;
...
static uint8_t *_get_jpeg_data_minifasnet
(
    vsi_nn_tensor_t *tensor,
    vnn_input_meta_t *meta,
    const char *filename
...
//Add by yingwei for integration, 20241108
static uint8_t *_get_jpeg_data_resnet34
    (
    vsi_nn_tensor_t *tensor,
    vnn_input_meta_t *meta,
    const char *filename,
    float* rgbData //Add by yingwei    
    )
{
    uint32_t i;
    uint8_t *data;
...
//Mod by yingwei
    ...
    {
        //data = bmpData;
        data = _float32_to_dtype(bmpData, tensor);
        TEST_CHECK_PTR(data, final);        
        goto final;
    }
...
static vsi_status _handle_multiple_inputs_minifasnet
(
    vsi_nn_graph_t *graph,
    uint32_t idx,
    const char *input_file
)
{
    ...
    {
    case NN_FILE_JPG:
        data = _get_jpeg_data_minifasnet(tensor, &meta, input_file);
        TEST_CHECK_PTR(data, final);
        break;
    case NN_FILE_TENSOR:
        data = _get_tensor_data(tensor, input_file);
...
//Add by yingwei for integration, 20241108
static vsi_status _handle_multiple_inputs_resnet34
    (
    vsi_nn_graph_t *graph,
    uint32_t idx,
    const char *input_file,
    ...
    )
{
    ...
    // Initialize tensor and metadata
    tensor = vsi_nn_GetTensor(graph, graph->input.tensors[idx]);
    if (tensor != NULL)
    {
        printf("Tensor dtype vx_type(hex): 0x%X\n", tensor->attr.dtype.vx_type);
    }
    ...
    {
    case NN_FILE_JPG:
        data = _get_jpeg_data_resnet34(tensor, &meta, input_file, rgbData);
        TEST_CHECK_PTR(data, final);  // Ensure data is not NULL before proceeding
        break;
    case NN_FILE_TENSOR:
        data = _get_tensor_data(tensor, input_file);
        TEST_CHECK_PTR(data, final);
    ...
    default:
        printf("error input file type\n");
        goto final; // Ensure proper handling of invalid file types
    }

    // Copy the pre-processed data to the input tensor
    status = vsi_nn_CopyDataToTensor(graph, tensor, data);
    TEST_CHECK_STATUS(status, final);

    // Convert tensor data to float32
    ...
    )
{
    ...
    {
        status = _handle_multiple_inputs_minifasnet(graph, i, inputs[i]);
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessFacedetector3Uint8
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num,
...
//Add by yingwei for integration, 20241108
vsi_status vnn_PreProcessResnet34Fp16
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num,
    ...
    )
{
    ...
    for(i = 0; i < input_num; i++)
    {
        status = _handle_multiple_inputs_resnet34(graph, i, inputs[i], rgbData);
        TEST_CHECK_STATUS(status, final);
    }
