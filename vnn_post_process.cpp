...
/*-------------------------------------------
                  Functions
-------------------------------------------*/
cv::Rect scale_and_clamp_bbox(const cv::Rect& bbox, float scale, int img_w, int img_h)
{
    float cx = bbox.x + bbox.width / 2.0f;
    float cy = bbox.y + bbox.height / 2.0f;
    float new_w = bbox.width * scale;
    float new_h = bbox.height * scale;
    int x = int(cx - new_w / 2.0f);
    int y = int(cy - new_h / 2.0f);
    int w = int(new_w);
    int h = int(new_h);
    // Clamp
    if (x < 0) x = 0;
...
// Hàm chuyển ảnh sang mảng input cho model (giả sử dùng uint8_t, BGR->RGB)
void Mat_to_array_anti_spoofing(const cv::Mat& input, uint8_t* pRgb)
{
    cv::Mat rgb;
    cv::cvtColor(input, rgb, cv::COLOR_BGR2RGB);
    memcpy(pRgb, rgb.data, 80 * 80 * 3);
}
